...
_LOGGER.info(f"Elpris Kvart Sensor Module Loaded - Version: {SENSOR_VERSION}")
...
async def async_setup_entry(
    ...
    _LOGGER.debug(
        f"Added {len(sensors_to_add)} Elpris Timme sensor entities. Version: {SENSOR_VERSION}"
    )
...
class BaseElprisSensor(CoordinatorEntity[ElprisDataUpdateCoordinator], SensorEntity):
    ...
    async def async_added_to_hass(self) -> None:
        ...
        _LOGGER.info(
            f"Sensor {self.entity_id} (Name: {self.name}, Unique ID: {self.unique_id}) added to HASS."
        )
        ...
        if self.coordinator.last_update_success and self.coordinator.data:
            _LOGGER.debug(
                f"Sensor {self.unique_id}: Coordinator has data on add, forcing update."
            )
            self._update_internal_data(write_state=True)
    ...
    def _calculate_raw_current_spot_price_sek(self) -> None:
        ...
        if self.coordinator.data:
            ...
            if prices_for_today_list:
                ...
                for price_info in prices_for_today_list:
                    try:
                        time_start_dt_local = dt_util.parse_datetime(
                            price_info["time_start"]
                        )
        ...
        _LOGGER.debug(
            f"Sensor {self.unique_id}: Calculated raw_spot_price_sek: {self._raw_current_spot_price_sek}"
        )
    ...
    def _schedule_next_hourly_price_update(self) -> None:
        ...
        next_update_time = (
            ...
            if now >= current_hour_target_update
            else current_hour_target_update
        )
        self._unsub_timer = async_track_point_in_time(
            self.hass, self._async_hourly_price_update_callback, next_update_time
        )

    async def _async_hourly_price_update_callback(
        self, now_triggered: DateTime
    ) -> None:
        self._update_internal_data(write_state=True)
        self._schedule_next_hourly_price_update()
...
class ElprisSpotSensorOre(BaseElprisSensor):
    ...
    def _update_sensor_specific_data(self) -> None:
        ...
        if (
            ...
            and self.coordinator.last_api_call_timestamp
        ):
        ...
        if self.coordinator.data:
            ...
            if attrs[ATTR_TOMORROW_PRICES_ORE]:
                ore_values = [
                    p["ore_per_kWh"]
                    for p in attrs[ATTR_TOMORROW_PRICES_ORE]
                    if "ore_per_kWh" in p
                ]
                if ore_values:
    ...
    def _update_sensor_specific_data(self) -> None:
        ...
        if self.coordinator.data:
            ...
            if attrs[ATTR_TOMORROW_PRICES_SEK]:
                sek_values = [
                    p["SEK_per_kWh"]
                    for p in attrs[ATTR_TOMORROW_PRICES_SEK]
                    if "SEK_per_kWh" in p
                ]
                if sek_values:
...
class ElprisInklusivePaslagSensorSEK(BaseElprisSensor):
    ...
    def _get_surcharge_sek_from_config(self) -> float:
        return round(
            self._get_surcharge_ore_from_config() / 100.0, SEK_ROUNDING_DECIMALS
        )
...
class SurchargeDisplaySensorBase(SensorEntity):
    ...
    def __init__(self, entry: ConfigEntry, price_area: str):
        ...
        _LOGGER.debug(
            f"Initialized {self.name} (Unique ID: {self.unique_id}), Surcharge: {self._attr_native_value}"
        )
...
class SurchargeOreSensor(SurchargeDisplaySensorBase):
    ...
    def _update_surcharge_value(self) -> None:
        ...
        _LOGGER.debug(
            f"{self.name}: Updated surcharge value to {self._attr_native_value} öre/kWh"
        )
...
class SurchargeSEKSensor(SurchargeDisplaySensorBase):
    ...
    def _update_surcharge_value(self) -> None:
        ...
        _LOGGER.debug(
            f"{self.name}: Updated surcharge value to {self._attr_native_value} SEK/kWh"
        )
